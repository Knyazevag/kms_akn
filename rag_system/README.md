...
- **Веб-интерфейс** на Gradio с историей диалога и памятью контекста; в панели
  «Обслуживание базы знаний» — кнопки **«Индексировать новые файлы»** и
  **«Сформировать заметки»** с индикатором прогресса (%); кнопка заметок
  активируется после успешной индексации.
...
- **Инкрементальная индексация** — неизменённые файлы пропускаются без повторного
  ...
  повторный прогон по большому архиву занимает секунды, а не часы.
