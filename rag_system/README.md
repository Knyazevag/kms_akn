...
- **Веб-интерфейс** на Gradio с историей диалога и памятью контекста; в панели
  «Обслуживание базы знаний» — кнопки **«Индексировать новые файлы»**,
  **«Сформировать заметки»** и **«Синхронизировать удаления»** с индикатором
  прогресса (%); кнопка заметок активируется после успешной индексации.
...
- **Инкрементальная индексация** — неизменённые файлы пропускаются без повторного
  парсинга (манифест `chroma_db/ingest_manifest.json` по `размер:mtime`), поэтому
  повторный прогон по большому архиву занимает секунды, а не часы. По завершении
  пишется файл-флаг `logs/ingest_status.txt`.
- **Синхронизация удалений** — удаление файла из архива убирает его чанки из
  индекса и заметку Obsidian (`ingest.py --prune` / кнопка в UI; наблюдатель
  реагирует на удаление автоматически). Есть защита: при пустом/недоступном
  архиве (отмонтирован диск) prune отменяется, чтобы не снести весь индекс.
- **Интеграция с Obsidian** — автогенерация заметок с YAML-frontmatter,
  тегами из таксономии (лимит 15 из словаря + 5 авто на заметку) и графом
  wikilinks по IDF-весу (топ-15 близких заметок), а также запросы к RAG прямо
  из заметок. Генерация устойчива: бюджет JSON авто-растёт, метаданные санируются.
...
python ingest.py --prune --prune-notes # индексация + удаление данных пропавших файлов
python ingest.py --prune-only --dry-run --prune-notes  # показать, что удалится (без удаления)
```

Повторный запуск **не переиндексирует всё**. Неизменённые файлы пропускаются на
уровне файла без повторного парсинга — по манифесту `chroma_db/ingest_manifest.json`
(отпечаток `размер:mtime`); парсятся только новые и изменённые. Дополнительно
действует дедуп на уровне чанков по хэшу. Прогон без изменений занимает секунды.
`--reset` очищает и базу, и манифест.
...
анализа, создаётся `.md` с YAML-frontmatter. Генерация устойчива: бюджет токенов
JSON авто-растёт на длинных ответах (`done_reason=length` → удвоение), а
метаданные санируются (отсев `null`/пустых), поэтому один документ не обрывает
весь прогон. Теги ограничены (15 из таксономии + 5 авто на заметку), чтобы модель
не вырождалась в длинные повторяющиеся списки.

Вторым проходом строятся wikilinks: для каждой заметки берутся **топ-15** самых
близких — общие теги взвешиваются по **IDF** (`log(N/df)`), поэтому редкий
специфичный тег важнее вездесущего (`co2_storage` встречается у сотен заметок);
порог — ≥ `WIKILINK_MIN_COMMON_TAGS` общих тегов.
...
| `TAXONOMY` | ~140 тегов | Контролируемый словарь тегов для Obsidian-заметок (LLM выбирает только из него; вне словаря — авто-теги с префиксом `auto/`) |
| `WIKILINK_MIN_COMMON_TAGS` | 2 | Минимум общих тегов для связи между заметками |
| `MAX_HISTORY_MESSAGES` | 6 | Глубина памяти диалога |

> Лимиты тегов и графа связей заданы в `doc_to_obsidian.py`: `MAX_TAXONOMY_TAGS=15`,
> `MAX_AUTO_TAGS=5` (тегов на заметку), `RELATED_TOP_N=15` (связей на заметку).
> Манифест инкрементальной индексации — `chroma_db/ingest_manifest.json`.
