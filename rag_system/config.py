...
RETRIEVAL_FETCH_K: int = 200          # размер пула кандидатов до балансировки
MIN_CHUNKS_PER_LANGUAGE: int = 2      # гарантированный минимум чанков на язык

# Дабл-запрос: переводить вопрос на второй язык (ru<->en) и искать обоими
# вариантами, объединяя пулы кандидатов. Решает корневую проблему перекоса
# multilingual-e5: при запросе на одном языке ближайшие соседи почти всегда
# одноязычны, и пул целиком оказывается на языке вопроса — балансировать нечего.
# Перевод запроса даёт нативную формулировку для второго языка, поэтому в пул
# попадают релевантные (а не «притянутые») источники другого языка.
# Перевод выполняется одним быстрым вызовом LLM-провайдера (Ollama).
CROSS_LANGUAGE_TRANSLATE_QUERY: bool = True
