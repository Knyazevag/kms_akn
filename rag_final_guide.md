...
LLM_MODEL:    str = "qwen2.5:7b"   # или "mistral", если скачали именно её
LLM_API_KEY:  str = ""
```

**Что означает каждая строка:**
- `LLM_PROVIDER` — активный провайдер LLM (подробнее — в Разделе 7).
- `LLM_MODEL` — имя модели для выбранного провайдера. Если в разделе 1.5 вы скачали `mistral`, укажите `"mistral"`; если `qwen2.5:7b` — оставьте как есть.
...
**Версия документа:** 5.0 (финальная) | **Год:** 2026  
**Автор:** Александр Князев, начальник ОТД АО «НИИ НПО «ЛУЧ» - ПФ  
