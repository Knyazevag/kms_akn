...
> [i] **Кросс-языковая балансировка (RU/EN).** Модель эмбеддингов слегка
> предпочитает язык запроса, и при малом `top_k` источники на другом языке
> вытеснялись из выдачи (например, на русский вопрос не попадали англоязычные
> статьи). Поэтому поиск идёт в два этапа: достаётся широкий пул кандидатов
> (`RETRIEVAL_FETCH_K = 40`), затем в финальную выдачу гарантированно попадает
...
> языке, чужой язык не навязывается. Отключается флагом `CROSS_LANGUAGE_BALANCE = False`
> в `config.py`. Проверить распределение языков в выдаче:
